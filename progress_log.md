...
### 22nd Feb 26 Progress: Software Trace Decoder (Day 10)
* **Objective:** Close the loop on the E-Trace ecosystem by building a host-side software tool to decode the raw binary packets generated by the hardware trace encoder.
* **Implementation:**
    * Modified the Verilator SystemVerilog testbench (`tb_encoder.sv`) to output raw, packed 80-bit hex strings to a file (`trace_output.hex`) instead of human-readable console prints.
    * Developed a Python decoding script (`decode.py`) using bitwise operations to unpack the Nexus struct (`mcode`, `timestamp`, `src_id`, `addr`).
* **Verification:** The Python script successfully parsed the output of the Verilated C++ model, calculating dynamic offsets and flawlessly reconstructing the 64-bit execution path (PC) of the simulated CPU. This validates the full-stack architecture from RTL generation to software reconstruction.
